...
BitStream JPEG(std::vector<ImageChannel<u8>> RGB_data) {
    /* Color Transform */
    auto YCbCr_data = rgb_to_ycbcr(RGB_data);

    /* Downsampling */
    auto Cb_downsampled = subsampling(YCbCr_data[1], SubsamplingMode(4, 4, 4));
    auto Cr_downsampled = subsampling(YCbCr_data[2], SubsamplingMode(4, 4, 4));
